			       ...
			       const u8 word, u8 *value,const u8 bank)
{
	...
	{					
#ifdef DEBUB												
		printk("rt2800_MT7630_rfcsr_read: RF read R%d=0x%X fail, i[%d], k[%d]\n", word, rfcsr.word,i,k);
#endif
		goto done;
	}
	ret = 0;
			 ...
			 const u8 *data, const size_t len)
{
	...
	if (rt2x00_rt(rt2x00dev, MT7630))
	{
		...
		printk("rt2800_load_firmware: %s: COM_REG0(0x%x) = 0x%x\n", __FUNCTION__, 0x0730, reg);

		if (reg != 0x1)
		{
			rt2800_register_read(rt2x00dev, 0x0700, &reg);
			printk("rt2800_load_firmware: %s: 0x0700 = 0x%x\n", __FUNCTION__, reg);

			rt2800_register_read(rt2x00dev, 0x0704, &reg);
			printk("rt2800_load_firmware: %s: 0x%x = 0x%x\n", __FUNCTION__, 0x0704, reg);
	...
	printk("rt2800_load_firmware: exit.\n");
...
#ifdef DEBUG
	printk("rt2800_config_channel_rf7630: exit.\n");
#endif	
}
...
#ifdef DEBUG
		//printk("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n");
		printk("rt2800_config_channel: ===>rt2800_config_channel channel=%d\n",rf->channel);
...
#ifdef DEBUG
		printk("rt2800_config_channel: exit.\n");
#endif	
}
