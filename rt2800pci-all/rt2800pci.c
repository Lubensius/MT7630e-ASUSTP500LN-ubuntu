/*
	For MT7630E Debugged      2014 Lubensius 
...
#ifdef DEBUG
static int water_irq = 0;
static int water_toggle = 0;
static int water_irq_enable = 0;
#endif
...
#ifdef DEBUG
		printk("rt2800pci_stop_queue: QID_BEACON\n");
#endif
		break;
	default:
		break;
...
#ifdef DEBUG
//extern static int water_toggle;
water_toggle++;
printk("rt2800pci_toggle_irq: init.\n");
#endif	
	/*
...
#ifdef DEBUG
		printk("rt2800pci_toggle_irq: set INT_MASK_CSR = 0x%x\n",reg);
#endif	
...
#ifdef DEBUG
		printk("rt2800pci_toggle_irq: 0x1300 = %08x\n", reg);
...
#ifdef DEBUG
		printk("rt2800pci_toggle_irq: %s(2):Check if PDMA is idle!\n", __FUNCTION__);
#endif	
		AsicWaitPDMAIdle(rt2x00dev, 5, 10);
	}
...
#ifdef DEBUG
if (water_toggle-- != 1) {
	printk("rt2800pci: rt2800pci_toggle_irq()               water: %d\n", water_toggle); 
}
printk("rt2800pci_toggle_irq: exit.\n");
#endif	
...
#ifdef DEBUG
//extern static int water_irq_enable;
water_irq_enable++;
#endif		
	/*
	...
	spin_unlock_irqrestore(&rt2x00dev->irqmask_lock, flags);
#ifdef DEBUG
if (water_irq_enable-- != 1) {
	printk("rt2800pci: rt2800pci_enable_interrupt()         water: %d\n", water_irq_enable); 
}
#endif	
}
...
static void rt2800pci_txstatus_tasklet(unsigned long data)
{
...
#ifdef DEBUG
	printk("rt2800pci_txstatus_tasklet: init.\n");
#endif
	 ...
	 * leave it enabledrivers/net/wireless/wl3501_cs.cd to minimize the possibility of a tx status
	 * register overflow. See comment in interrupt handler.
	 */
#ifdef DEBUG
	printk("rt2800pci_txstatus_tasklet: exit.\n");
#endif
}
...
#ifdef DEBUG
	printk("rt2800pci_tx8damdone_tasklet: init.\n");
#endif

	if (RTMPHandleTxRing8DmaDoneInterrupt(rt2x00dev)) {			//eigendlich liefert die Fkt nichts zurück???
	#ifdef DEBUG
		printk("rt2800pci_tx8damdone_tasklet: RTMPHandleTxRing8DmaDoneInterrupt.\n");
	#endif
...
#ifdef DEBUG
	printk("rt2800pci_tx8damdone_tasklet: exit.\n");
#endif
...
#ifdef DEBUG
	printk("rt2800pci_pretbtt_tasklet: init.\n");
#endif
...
#ifdef DEBUG
	printk("rt2800pci_pretbtt_tasklet: exit.\n");
#endif
}
...
#ifdef DEBUG
	printk("rt2800pci_tbtt_tasklet: init.\n");
#endif
...
#ifdef DEBUG
	printk("rt2800pci_tbtt_tasklet: exit.\n");
#endif
}
...
static void rt2800pci_rxdone_tasklet(unsigned long data)
{
	struct rt2x00_dev *rt2x00dev = (struct rt2x00_dev *)data;
#ifdef DEBUG
	printk("rt2800pci_rxdone_tasklet: init.\n");
#endif
...
#ifdef DEBUG
	printk("rt2800pci_rxdone_tasklet: exit.\n");
#endif
}
...
static void rt2800pci_autowake_tasklet(unsigned long data)
{
	struct rt2x00_dev *rt2x00dev = (struct rt2x00_dev *)data;
#ifdef DEBUG
	printk("rt2800pci_autowake_tasklet: init.\n");
#endif
...
#ifdef DEBUG
	printk("rt2800pci_autowake_tasklet: exit.\n");
#endif
}
...
static void rt2800pci_txstatus_interrupt(struct rt2x00_dev *rt2x00dev)
{
...
#ifdef DEBUG
	printk("rt2800pci_txstatus_interrupt: init.\n");
#endif		
	/*
	...
	for (i = 0; i < rt2x00dev->ops->tx->entry_num; i++) {
		...
		{
			#ifdef DEBUG
			printk("rt2800pci_txstatus_interrupt: !rt2x00_get_field32(TX_STA_FIFO, TX_STA_FIFO_VALID) [%d]\n", i);
			#endif
			break;
		}		
#endif
		...
		#ifdef DEBUG
		printk("rt2800pci_txstatus_interrupt: 0x%x (status) count:%d\n",Fifi_Status.word,i);
		#endif
		status = Fifi_Status.word;

		//woody_kfifo_put(&rt2x00dev->txstatus_fifo, &status);
		if (!kfifo_put(&rt2x00dev->txstatus_fifo, status)) {			//Falsch ? nein, sollte es &status sein? nein
			#ifdef DEBUG
			WARNING(rt2x00dev, "rt2800pci_txstatus_interrupt: TX status FIFO overrun,"
				"drop tx status report.\n");
			printk("rt2800pci_txstatus_interrupt: TX status FIFO overrun, drop tx status report\n");
			#endif
			break;
		}
	}
	...
	tasklet_schedule(&rt2x00dev->txstatus_tasklet);
#ifdef DEBUG
	printk("rt2800pci_txstatus_interrupt: exit.\n");
#endif		
...
#ifdef DEBUG
//extern static int water_irq;
	water_irq++;
	printk("rt2800pci_interrupt: init.\n");
#endif		
	...
	if (!reg) {
		#ifdef DEBUG
		water_irq--;
		printk("rt2800pci_interrupt: NONE.\n");
		#endif		
		return IRQ_NONE;
	}
	if (!test_bit(DEVICE_STATE_ENABLED_RADIO, &rt2x00dev->flags)) {
		#ifdef DEBUG
		water_irq--;
		printk("rt2800pci_interrupt: RADIO off.\n");
		#endif		
		return IRQ_HANDLED;
	}
	/*
...
#ifdef DEBUG
	if (water_irq-- != 1) {
		printk("rt2800pci: rt2800pci_interrupt()                water: %d\n", water_irq); 
	}
	printk("rt2800pci_interrupt: exit.\n");
#endif	
	return IRQ_HANDLED;
}
