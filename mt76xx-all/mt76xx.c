 ...
 *
 *  Changed by Lubensius 2015 to work on ASUS TP500LN
 *  only relevant entry in mtk_bt_loader_tbl[]
 *  Version number hack to satisfy dkms
*/
...
static void mtk_loader(struct mtk_loader_date *data)
{
	...
	if (is_mt7630(data) || is_mt7650(data)) {
		...
		if (ret < 0) {
		    ...
		    printk ("Firmware file 2 \"%s\" Found \n", 
				    MT76x2_FIRMWARE);
		    /* load rom patch here */
		    data->firmware = firmware;
...
//	{ USB_DEVICE_AND_INTERFACE_INFO(0x0e8d, 0x763f, 0xe0, 0x01, 0x01) },
//	{ USB_DEVICE_AND_INTERFACE_INFO(0x0e8d, 0x763f, 0xe0, 0x01, 0x01) },
	/* Mediatek MT662 */
